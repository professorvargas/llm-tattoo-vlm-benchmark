...
IN_JSONL = Path("runs/gemma3/crops_black/test_closed/raw/p1_crop_test_closed.jsonl")

OUT_JSONL = Path("runs/qwen2_5_vl/crops_black/test_closed/raw/p1_crop_test_closed.jsonl")
OUT_LOGDIR = Path("runs/qwen2_5_vl/crops_black/test_closed/logs")
OUT_EVALDIR = Path("runs/qwen2_5_vl/crops_black/test_closed/eval")
...
with IN_JSONL.open("r", encoding="utf-8") as fin, OUT_JSONL.open("w", encoding="utf-8") as fout:
    for line in fin:
        ...
        if not crop_path or not os.path.exists(crop_path):
            # salva registro de erro, mas não para o run
            rec_out = {
        ...
        rec_out = {
            ...
            "seconds": round(dt, 6),
            # mantém alguns metadados úteis do original, se existirem:
            "split": rec_in.get("split", "test_closed"),
